...
class CreatePostView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            BlogPostPage.delete(form.instance, using="default")
...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if can_view_other_posts:
            ...
            context["other_posts"] = BlogPostPage.objects.filter(query).exclude(id=post.id).order_by(
                "-created_at"
            )[: random.randint(3, 5)]
        else:
            context["other_posts"] = []
...
class AuthorPostsView(ListView):
    ...
    def get_queryset(self):
        ...
        posts = BlogPostPage.objects.filter(author=self.author)
...
def edit_blog_post(request, post_id):
    post = get_object_or_404(BlogPostPage, id=post_id, author=request.user)
