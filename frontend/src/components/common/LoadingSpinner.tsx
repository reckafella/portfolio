    ...
    return (
        <div className={`text-center ${className}`}>
            <div className="gap-2 d-flex align-items-center justify-content-center">
                ...
                {text && size !== "sm" && <div className="mt-2">{text}</div>}
            </div>
        </div>
    );
};
...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({
    ...
    return size === "lg" ? (
        <LargerSpinner size={size} text={text} className={className} />
    ) : (
        <NormalSpinner size={size} text={text} className={className} />
    );
};
