...
export const FileUpload: React.FC<{
    ...
    multiple?: boolean;
    max_size?: number;
}> = ({ fieldName, value, onFileChange, fileBaseProps, accept, multiple, max_size }) => {
    ...
    return (
        <div className="file-upload-container">
            ...
            >
                ...
                <p className="mb-1"><strong>Drop files here or click to browse</strong></p>
                ...
                <div className="mt-3">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <button
                            ...
                            onClick={() => onFileChange(fieldName, null, multiple ?? false)}
                        >
                            ...
                            <div key={index} className="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        onFileChange(fieldName, newFiles, multiple ?? false);
                                    }}
                                >
...
export const ImageUpload: React.FC<{
    ...
    multiple?: boolean;
    max_size?: number;
}> = ({ fieldName, value, onFileChange, fileBaseProps, accept: _accept, multiple, max_size }) => {
    const imageValue = value as File | File[] | string;
    const selectedImages = useMemo(
        () => Array.isArray(imageValue) ? imageValue : (imageValue instanceof File ? [imageValue] : []),
        [imageValue]
    );
    ...
    const handleDrop = useCallback((e: React.DragEvent) => {
        ...
        if (isAtLimit) {
            alert(`You have already reached the maximum of ${MAX_IMAGES} images.`);
            return;
        }
        ...
        if (files.length > 0) {
            if (multiple) {
                ...
                if (newFiles.length > MAX_IMAGES) {
                    ...
                    onFileChange(fieldName, newFiles.slice(0, MAX_IMAGES), multiple);
                } else {
    ...
    return (
        <div className="image-upload-container">
            <div
                className="drag-drop-area border-2 border-dashed rounded p-4 text-center"
                style={{
                    borderColor: isAtLimit ? 'var(--text-error-color)' : 'var(--default-color)',
                    backgroundColor: isAtLimit ? 'var(--text-error-color)' : 'var(--card-background-color)',
                ...
                onDragLeave={(e) => {
                    e.preventDefault();
                    e.currentTarget.style.borderColor = isAtLimit ? 'var(--text-error-color)' : 'var(--default-color)';
                }}
                onDrop={handleDrop}
                onClick={() => {
                    if (isAtLimit) {
            ...
            >
                <i className="bi bi-cloud-arrow-up fs-1 text-muted mb-2"></i>
                <p className="mb-1"><strong>Drop images here or click to browse</strong></p>
                <p className="small text-muted mb-0">
