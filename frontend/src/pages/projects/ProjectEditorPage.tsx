...
export function ProjectEditorPage({
    ...
    const [showContentUpdatedWarning, setShowContentUpdatedWarning] =
        useState(false);
    ...
    useEffect(() => {
        if (initialProject && !initialDataLoaded && isEditing) {
            ...
            setFormData({
                ...
                images: [],
                youtube_urls: initialProject.videos?.map(
                    (v: { youtube_url: string }) => v.youtube_url,
                ) || [""],
                existing_images:
                    initialProject.images?.map(
                        (img: {
                            id: number;
                ...
                existing_videos:
                    initialProject.videos?.map(
                        (vid: { id: number; youtube_url: string }) => ({
                            id: vid.id,
    ...
    useEffect(() => {
        const handleTabSyncMessage = (message: TabSyncMessage) => {
            if (
                ...
                message.payload.editType === "project" &&
                message.payload.editSection === slug
            ) {
                ...
                message.payload.contentType === "project" &&
                message.payload.contentId === slug
            ) {
    ...
    useEffect(() => {
        ...
        if (
            ...
            formData.description !== initialContentRef.current &&
            shouldAutoSave(formData)
        ) {
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (totalImages > 5) {
            setErrors((prev) => ({
                ...prev,
                images: ["You can only upload up to 5 images total"],
            }));
            return;
        }
    ...
    const restoreExistingImage = (imageId: number) => {
        setFormData((prev) => ({
            ...prev,
            images_to_delete: (prev.images_to_delete || []).filter(
                (id) => id !== imageId,
            ),
        }));
    };
    ...
    const updateYoutubeUrl = (index: number, value: string) => {
        setFormData((prev) => ({
            ...prev,
            youtube_urls: prev.youtube_urls.map((url, i) =>
                i === index ? value : url,
            ),
        }));
    };
        ...
        try {
            ...
            if (error && typeof error === "object") {
                ...
                if (errorObj.status === 400 && errorObj.data) {
                    ...
                    setErrors({
                        general: ["Failed to save project. Please try again."],
                    });
                }
            } else {
                setErrors({
                                ...
                                <div className="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <h1 className="h4 mb-0">
                                            {isEditing
                                                ? `Edit: ${initialProject?.title || "Project"}`
                                                : "New Project"}
                                        </h1>
                                            ...
                                            formData.existing_images.length >
                                                ...
                                                <div className="mb-3">
                                                    ...
                                                    <div className="row g-2">
                                                        {formData.existing_images.map(
                                                            (img) => {
                                                                const isMarkedForDeletion =
                                                                    formData.images_to_delete?.includes(
                                                                        img.id,
                                                                    );
                                                                return (
                                                                    <div
                                                                    ...
                                                                    >
                                                                        <div
                                                                            className={`card position-relative ${isMarkedForDeletion ? "opacity-50" : ""}`}
                                                                        >
                                                                            <img
                                                                                src={
                                                                                    img.url
