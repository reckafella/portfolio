...
export const createDataForSubmission = (data: ProjectFormData): Record<string, string | number | boolean | File | File[]> => {
  const submitData: Record<string, string | number | boolean | File | File[]> = {
    title: data.title,
  ...
  if (data.images_to_delete && data.images_to_delete.length > 0) {
    submitData.delete_images = data.images_to_delete.map(id => String(id)).join(',');
  }

  // Add deletion support for videos
  if (data.videos_to_delete && data.videos_to_delete.length > 0) {
    submitData.delete_videos = data.videos_to_delete.map(id => String(id)).join(',');
  }
