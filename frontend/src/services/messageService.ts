...
export const messageService = {
   ...
   */
  async getMessages(filters: Partial<MessageFilters>): Promise<MessageListResponse> {
   ...
   */
  async markAsRead(messageId: number): Promise<{ success: boolean; message: string; is_read: boolean }> {
    const response = await apiRequest(`${BASE_URL}/${messageId}/mark-read/`, {
      method: 'POST'
    });
    return response.json();
  },
   ...
   */
  async markAsUnread(messageId: number): Promise<{ success: boolean; message: string; is_read: boolean }> {
    const response = await apiRequest(`${BASE_URL}/${messageId}/mark-unread/`, {
      method: 'POST'
    });
    return response.json();
  },
